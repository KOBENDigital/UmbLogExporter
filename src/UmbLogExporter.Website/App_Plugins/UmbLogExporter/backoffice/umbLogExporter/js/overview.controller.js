(function () {
	...
	function controller($q, $location, $timeout, logViewerResource, navigationService, umbLogExporterResource) {
		...
		//custom
		vm.exportLogQuery = exportLogQuery;
		...
		vm.buttonGroup = {
			defaultButton: {
				...
				handler: function (logQuery) {
					searchLogQuery(logQuery);
				}
			},
			subButtons: [
				{
					labelKey: "actions_export",
					hotKey: "",
					hotKeyWhenHidden: true,
					handler: function (logQuery) {
						exportLogQuery(logQuery);
					}
				}
			]
		};
		...
		function searchLogQuery(logQuery) {
			$location.path("/settings/umbLogExporter/search").search({
				lq: logQuery,
		...
		//custom
		function exportLogQuery(logQuery) {
			umbLogExporterResource.exportLogs(vm, {
				filterExpression: logQuery,
				startDate: vm.startDate,
				endDate: vm.endDate
			});
		}
		...
		vm.dateRangeChange = function (selectedDates, dateStr, instance) {
			...
			if (selectedDates.length > 0) {
				...
				$location.path("/settings/umbLogExporter/overview").search({
					startDate: startDate,
					endDate: endDate
				});
			}
	...
	angular.module("umbraco").controller("UmbLogExporter.Controllers.OverviewController", controller);
})();